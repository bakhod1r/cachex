...
func conn(t *testing.T) *amqp.Connection {
	t.Helper()
...
func newInv(t *testing.T, c *amqp.Connection, ex string) *Invalidator {
	t.Helper()
	inv, err := New(Config{Conn: c, Exchange: ex, OnError: func(err error) { t.Logf("OnError: %v", err) }})
	if err != nil {
		t.Fatal(err)
	}
...
func TestIntegrationPubSub(t *testing.T) {
	ex := exchangeName()
	a := newInv(t, conn(t), ex)
	b := newInv(t, conn(t), ex)
	...
	// Published immediately after Subscribe returned: must be delivered.
